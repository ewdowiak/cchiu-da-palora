...
my $vnhash = retrieve('../cgi-lib/vocab-notes-plus' );
my %vnotes = %{ $vnhash };

##  retrieve Traina dictionary and tools
my $ttline = retrieve('../cgi-lib/traina_line-to-traina' );
my %lt_hash = %{ $ttline };

my $tthead = retrieve('../cgi-lib/traina_hdword-to-line' );
my %hl_hash = %{ $tthead };

my $ttspan = retrieve('../cgi-lib/traina_span-to-hdword' );
my %sh_hash = %{ $ttspan };

my $ttlist = retrieve('../cgi-lib/traina-list' );
...
get '/' => sub ($c) {
    ...
    my $par_traina = $c->param('traina');
    my $output = mk_htmlpage( $par_palora , $par_langs , $par_traina );
...
sub mk_htmlpage{ 
	...
	## make top
	$htmlpage .= mk_cctophtml("../config/eryk2-topnav.html" , "" , \%vnotes , $vbconj );
	...
	## make top
	$htmlpage .= mk_cctophtml("../config/eryk2-topnav.html" , $inword , \%vnotes , $vbconj );
	$htmlpage .= mk_newform( $lgparm );
    ...
    my $text_url   = 'https://www.napizia.com/cgi-bin/cchiu-da-palora.pl';
    my $text_title;
    if ( ! defined $inword || ! defined $vnotes{ $inword } ) {
	my $blah  = 'do nothing';
    } else { 
    ...
    ##  print footer
    $htmlpage .= mk_foothtml("../config/eryk2-navbar.html");
